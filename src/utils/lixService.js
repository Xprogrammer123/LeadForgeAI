...
const lixService = {
  ...
  searchLeads: async (filters) => {
    try {
      ...
      const response = await fetchWithRetry(`https://api.lix-it.com/v1/leads/search?${queryParams}`, {
        method: 'GET',
        headers: {
  ...
  sendLinkedInMessage: async (leadId, message, campaignId) => {
    try {
      ...
      const response = await fetchWithRetry('https://api.lix-it.com/v1/messages/send', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${apiKey}`,
          'Content-Type': 'application/json'
  ...
  getInboxMessages: async (campaignId) => {
    try {
      ...
      const response = await fetchWithRetry(`https://api.lix-it.com/v1/messages/inbox?campaign_id=${campaignId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${apiKey}`,
          'Content-Type': 'application/json'
  ...
  createLixCampaign: async (campaignData) => {
    try {
      ...
      const response = await fetchWithRetry('https://api.lix-it.com/v1/campaigns', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${apiKey}`,
          'Content-Type': 'application/json'
  ...
  /**
   * Get campaign performance metrics from Lix
   */
  getCampaignMetrics: async (lixCampaignId) => {
    try {
      const apiKey = validateApiKey();
      
      const response = await fetchWithRetry(`https://api.lix-it.com/v1/campaigns/${lixCampaignId}/metrics`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${apiKey}`,
          'Content-Type': 'application/json'
  ...
  validateConnection: async () => {
    try {
      ...
      const response = await fetchWithRetry('https://api.lix-it.com/v1/auth/validate', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${apiKey}`,
          'Content-Type': 'application/json'
