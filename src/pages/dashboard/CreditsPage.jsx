...
function CreditsPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (user?.id) {
        ...
        const creditStatsResult = await creditService.getCreditStats(
          user.id,
          "30_days"
        );
        ...
        const transactionsResult = await creditService.getCreditTransactions(
          user.id,
          30
        );
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="glassmorphism rounded-lg p-3 border border-border">
          ...
          {data.usage > 0 && (
            <p className="text-xs text-warning">Used: -{data.usage} credits</p>
          )}
  ...
  if (loading) {
    return (
      <DashboardLayout title="Dashboard" currentPath="/credits">
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <StatCard
            ...
            color="warning"
            change={
              creditStats.totalPurchased > creditStats.totalUsed
                ? Math.round(
                    ((creditStats.totalPurchased - creditStats.totalUsed) /
                      Math.max(creditStats.totalUsed, 1)) *
                      100
                  )
                : -Math.round(
                    ((creditStats.totalUsed - creditStats.totalPurchased) /
                      Math.max(creditStats.totalPurchased, 1)) *
                      100
                  )
            }
          />
              ...
              <div className="h-full flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Purchase credits or create campaigns to see your activity
                    here
                  </p>
                </div>
              </div>
        ...
        {/* Purchase Section (Replaces Modal) */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {stripeService.getCreditPackages().map((pkg) => (
              <div
                key={pkg.id}
                className={`border rounded-xl p-6 glassmorphism shadow-md ${
                  pkg.popular ? "border-primary/50" : "border-border/30"
                }`}
              >
                {pkg.popular && (
                ...
                <h4 className="text-lg font-headline-bold text-foreground mb-1">
                  {pkg.name}
                </h4>
                <p className="text-2xl font-headline-bold text-foreground mb-2">
                ...
                <ul className="text-sm text-muted-foreground mb-4 space-y-1">
                  <li>• {pkg.campaigns}</li>
                  <li>• {pkg.credits} credits included</li>
                  {pkg.savings && <li>• Save {pkg.savings}</li>}
                </ul>
                <Button
                  onClick={() => handlePurchaseCredits(pkg)}
                  variant={pkg.popular ? "default" : "outline"}
                  className={pkg.popular ? "w-full cta-button" : "w-full"}
                >
                  Purchase Credits
                </Button>
              </div>
